...
ExecutionFunction = Callable[[List[UQModel], List[np.ndarray]],
                             List[np.ndarray]]


def simple_serial_execution(models: List[UQModel],
...
def check_model_execution(execution_function: ExecutionFunction,
                          num_models: int, max_cost: float,
                          model_cost_ratio: float, cost_std_ratio: float,
                          target_cost: float, num_processes: int = 1):
    """
    Runs a model execution function in a monomial model test case.
    ...
    :param execution_function: The function which executes the models
    ...
    t_start = time.time()
    model_outputs = execution_function(models, model_inputs)
    t_end = time.time()
    ...
    print("Monomial Model Execution Results:")
...
if __name__ == "__main__":
    ...
    EXECUTION_FUNCTION = simple_serial_execution
